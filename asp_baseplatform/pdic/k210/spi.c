 ...
 */
static void
spi_send_data_normal(SPI_Handle_t *hspi, int8_t ss_no, const uint8_t *tx_buff, size_t tx_len)
{
	size_t fifo_len;
	const uint32_t *src = (const uint32_t *)tx_buff;
	...
	while (tx_len > 0) {
		...
		uint32_t *dst = (uint32_t *)(hspi->base + TOFF_SPI_DR);
		uint32_t *end = &dst[fifo_len];
		for (; dst < end; src++, dst++) {
			sil_wrw_mem(dst, *src);
		}
		tx_len -= fifo_len;
 ...
 */
static void
spi_send_data_normal2(SPI_Handle_t *hspi, int8_t ss_no, uint32_t tx_data, size_t tx_len)
{
	size_t fifo_len;
	...
	{
	case 2:
		tx_data = (tx_data << 16) | (tx_data & 0xFFFF);
		break;
	case 1:
		tx_data = (tx_data << 24) | ((tx_data << 16) & 0xFF0000) | ((tx_data << 8) & 0xFF00) | (tx_data & 0xFF);
		break;
	}
...
spi_core_transmit(SPI_Handle_t *hspi, int8_t ss_no, uint8_t *pdata, uint16_t length)
{
	...
	if (hspi->hdmatx != NULL) {
		...
		spi_send_data_normal(hspi, ss_no, (const void *)pdata, length);
	}
...
spi_core_transmit_fill(SPI_Handle_t *hspi, int8_t ss_no, const uint32_t *tx_buff, size_t tx_len)
{
	...
	if (hspi->hdmatx != NULL) {
		...
		hdmatx = spi_dmac_set_single_mode(hspi, 0, ss_no, tx_buff,
			(void *)(hspi->base + TOFF_SPI_DR), DMAC_ADDR_NOCHANGE, DMAC_ADDR_NOCHANGE,
			DMAC_MSIZE_1, DMAC_TRANS_WIDTH_32, tx_len);
		spi_dmac_wait_done(hdmatx);
	}
	else {
		spi_send_data_normal2(hspi, ss_no, *tx_buff, tx_len);
	}
#if SPI_WAIT_TIME != 0
...
spi_core_receive(SPI_Handle_t *hspi, int8_t ss_no, void *rx_buff, size_t rx_len)
{
	...
	hdmarx = spi_dmac_set_single_mode(hspi, 1, ss_no,
		(void *)(hspi->base + TOFF_SPI_DR), rx_buff, DMAC_ADDR_NOCHANGE, DMAC_ADDR_INCREMENT,
...
spi_core_transrecv(SPI_Handle_t *hspi, int8_t ss_no, const uint8_t *tx_buf, uint8_t *rx_buf, size_t len)
{
	...
	if (hspi->hdmatx != NULL) {
		...
		hdmarx = spi_dmac_set_single_mode(hspi, 1, ss_no,
			(void *)(hspi->base + TOFF_SPI_DR), rx_buf, DMAC_ADDR_NOCHANGE, DMAC_ADDR_INCREMENT,
			DMAC_MSIZE_1, DMAC_TRANS_WIDTH_32, v_len);
		spi_send_data_normal(hspi, -1, (const void *)tx_buf, len);
	}
	spi_dmac_wait_done(hdmarx);
