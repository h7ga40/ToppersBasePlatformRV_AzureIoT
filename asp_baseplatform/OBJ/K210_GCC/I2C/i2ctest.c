...
/* 
 *  I2Cテストの本体
 *  本テストはmaxi bitにブレッドボードにI2Cシールドを接続してテスト
 *  maxidunoでは電気的な問題で通信エラーとなる.
 */
...
void main_task(intptr_t exinf)
{
	...
	syslog_1(LOG_NOTICE, "I2C SEND/RECV START(%d)", I2C_PORTID);

	i2c_initd.ClockSpeed      = 100000;
	i2c_initd.OwnAddress1     = 0;
	i2c_initd.AddressingMode  = I2C_ADDRESSINGMODE_7BIT;
	i2c_initd.SclPin          = getGpioPin(15);
	i2c_initd.SdaPin          = getGpioPin(14);
	...
	syslog_1(LOG_NOTICE, "LCD INITAILIZE(%d) !", I2C_PORTID);
	...
	do{
		...
		temp = aRxBuffer[0];
		syslog_1(LOG_NOTICE, "## temp[%02x] ##", temp);
	}while((temp & 0x80) != 0);
